...
public class AlertsPage extends BasePage{
  ...
  public AlertsPage clickByAlertWithTimer() {
    ...
    new WebDriverWait(driver, Duration.ofSeconds(5))//ожидание alert
  ...
  public AlertsPage selectResult(String confirm) {
    ...
    if (confirm != null && confirm.equals("Cancel")) {
      driver.switchTo().alert().dismiss();// нажимаем на cancel при помощи dismiss
  ...
  public AlertsPage sendMessageToAlert(String message) {
    ...
    if (message != null) {
      driver.switchTo().alert().sendKeys(message);// можно послать текст в alert
      driver.switchTo().alert().accept();
    }
    return this;
  }
